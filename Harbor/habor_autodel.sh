...
rm -rf ${FILE_NAME}
read -p "請輸入登入帳號:" ACCOUNT
read -p "請輸入登入密碼"  PASSWD
...
REPO_COUNT=$(curl -sX GET "${PORT}://${IP}/api/v2.0/projects?name=${PROJECT_NAME}" -H "accept: application/json" -u "${ACCOUNT}:${PASSWD}"|jq -r '.[] | .repo_count') #get project count
PROJECT_LIST=$(curl -sX GET "${PORT}://${IP}/api/v2.0/projects/${PROJECT_NAME}/repositories?page_size=${REPO_COUNT}" -H "accept: applicaton/json" -u "${ACCOUNT}:${PASSWD}") #list all register name
  #echo ${PROJECT_LIST}
  ...
  do
    ...
    if [[ ${ARTIFACT_COUNT} -gt ${NUMBER} ]];then # if $ARTIFACT_COUNT > ${NUMBER} then try to delete images
	  ...
	  ARTIFACTS_LIST=$(curl -sX GET "${PORT}://${IP}/api/v2.0/projects/${PROJECT_NAME}/repositories/${TAG_NAME}/artifacts?page_size=${ARTIFACT_COUNT}" -H "accept: applicaton/json" -u "${ACCOUNT}:${PASSWD}")
      ...
      for  (( i=1;i<=DELETE_NUMBER;i++ ))
        ...
        DELETE_ID=$(cat ${FILE_NAME}|awk -F ',' '{print $4}'|tail -n ${i}|head -n 1) #id        
          ...
          if [[ ${DELETE_DAY} -gt 30  ]];then #than one month delete
            ...
            echo "curl -sX DELETE "${PORT}://${IP}/api/v2.0/projects/${PROJECT_NAME}/repositories/${TAG_NAME}/artifacts/${DELETE_ID}" -H "accept: application/json" -u "${ACCOUNT}:${PASSWD}""
          else 
